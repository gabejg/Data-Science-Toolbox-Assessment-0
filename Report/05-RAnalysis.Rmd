...
```{r}
qplot(dst_host_rerror_rate,dst_host_srv_rerror_rate,colour=normal,data=kddata)
```

"dst_host_rerror_rate" and "dst_host_srv_rerror_rate" are two strong predictors, when "dst_host_rerror_rate" = 1, intrusion type is "probe".


```{r}
qplot(num_root,num_shells,colour=normal,data=kddata)
```
...
```{r}
qplot(serror_rate,rerror_rate,colour=normal,data=kddata)
```

"serror_rate" and "rerror_rate" are two strong predictors, when "serror_rate" + "rerror_rate" =1 and "serror_rate" < 0.875, intrusion type is "probe", but when "serror_rate" > 0.875, intrusion type is "dos".
